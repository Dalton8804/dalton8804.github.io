...
export class Fish {
    ...
    resolve() {
        ...
        if (Target.dist(this._target, headPos) < 40 || this.timeSinceLastTargetHit > 400) {
            this._target.hit();
            this._target = this.#newTarget;
            this.timeSinceLastTargetHit = 0;
        }
    ...
    get #newTarget() {
        ...
        let temp = new Target(Math.random() * window.innerWidth, Math.random() * window.innerHeight);
        while (Target.dist(temp, this.spine.joints[0]) < 400) {
            temp = new Target(Math.random() * Math.max(1000, window.innerWidth), Math.random() * window.innerHeight);
        }
        return temp;
    }
